 ...
 *
 * A side drawer on desktop and a sheet on mobile — both read as a peek at one
...
export function ApplicationDrawer({ row, onClose }: Props) {
  ...
  return (
    <>
      {/* Mobile: sheet from the bottom edge. */}
      <BottomSheet label={`${row.company} — ${row.role}`} onClose={onClose}>
        <div className="flex flex-none items-start justify-between gap-3 border-b border-line2 px-5 pb-3.5 pt-4">
          <Title row={row} />
      ...
      {/* Desktop: drawer from the right edge. */}
      <div aria-hidden onClick={onClose} className="absolute inset-0 z-20 hidden bg-ink/[0.18] md:block" />
      <aside
        role="dialog"
        aria-modal="true"
        aria-label={`${row.company} — ${row.role}`}
        className="absolute inset-y-0 right-0 z-30 hidden w-[392px] flex-col border-l border-line bg-surface shadow-[-18px_0_40px_rgba(60,45,30,0.09)] md:flex"
      >
...
function Title({ row }: { row: ApplicationRow }) {
  return (
...
function CloseButton({ onClose }: { onClose: () => void }) {
  return (
    <button
      ...
      onClick={onClose}
      aria-label="Close"
      className="h-8 w-8 flex-none rounded-lg border border-line bg-surface leading-none text-muted hover:text-ink"
    >
      ×
    </button>
  );
...
function DrawerBody({ row }: { row: ApplicationRow }) {
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 overflow-y-auto px-5 pb-[max(1.75rem,env(safe-area-inset-bottom))] pt-4">
          ...
          <ol className="flex flex-col">
            ...
            <li className="grid grid-cols-[18px_1fr] gap-3">
              <span className="flex flex-col items-center">
                <span aria-hidden className="mt-1 h-[9px] w-[9px] rounded-full bg-[#e0d5c6]" />
              </span>
